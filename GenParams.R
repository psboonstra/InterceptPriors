...
if(!"exppow_prior_args"%in%ls()){
  exppow_prior_args = list(EPTwoAdapt = list(alpha_scale = NA,alpha_power = 2,adaptive_scale = T),
                           EPTenAdapt = list(alpha_scale = NA,alpha_power = 10,adaptive_scale = T),
                           #Fixed scale with 1% of prior mass following outside of |logit(1/1e-4)|=~9.21
                           EPTenFixed = list(alpha_scale = 6.218102,alpha_power = 10,adaptive_scale = F));
}
...
if(!"all_settings_list"%in%ls()){
  all_settings_list = list(
    ...
    list(scenario = 1, 
         ...
         x_binom = 1:4, x_skew_param = 0, case_control_ratio = NA,
    ...
    list(scenario = 2, 
         ...
         x_binom = 1:25, x_skew_param = 0, case_control_ratio = NA,
    ...
    list(scenario = 6, 
         hiershrink_prior_num_relevant = 10,
         true_alpha = -3,true_betas = rep(0.15, 25),
         x_binom = NULL, x_skew_param = 10, case_control_ratio = NA,
         n = 50 , mu_latent_x = NULL, chol_latent_x = chol(matrix(0.50,nrow=25,ncol=25)+diag(0.5,25))),
    list(scenario = 6, 
         hiershrink_prior_num_relevant = 10,
         true_alpha = -3,true_betas = rep(0.15, 25),
         x_binom = NULL, x_skew_param = 10, case_control_ratio = NA,
         n = 100 , mu_latent_x = NULL, chol_latent_x = chol(matrix(0.50,nrow=25,ncol=25)+diag(0.5,25))),
    list(scenario = 6, 
         hiershrink_prior_num_relevant = 10,
         true_alpha = -3,true_betas = rep(0.15, 25),
         x_binom = NULL, x_skew_param = 10, case_control_ratio = NA,
         n = 200 , mu_latent_x = NULL, chol_latent_x = chol(matrix(0.50,nrow=25,ncol=25)+diag(0.5,25))),
    #7
    list(scenario = 7, 
         hiershrink_prior_num_relevant = 10,
         true_alpha = -4,true_betas = c(2,rep(0,74)), 
         x_binom = NULL, x_skew_param = 0, case_control_ratio = NA,
    ...
    list(scenario = 8, 
         ...
         x_binom = 1:75, x_skew_param = 0, case_control_ratio = NA,
...
if(!"rep_settings"%in%ls()){rep_settings = c(rep(25,25),rep(100,8));}
...
random_seed = sample(.Machine$integer.max,length(all_settings_list), replace = F);
...
for(i in 1:length(all_settings_list)) {
  ...
  x_skew_param = all_settings_list[[i]]$x_skew_param;
  ...
  curr_params = list(niter = ifelse("niter"%in%ls(),niter,ifelse(length(true_betas) <= 100, 8, 2)),
                     n = n,
                     n_new = n_new,
